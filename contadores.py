...
import DBLoad, Counters, Printers
...
def getAttrsImpressoras(impressora):
    ...
    try:
        driver.get(f'https://{impressora["ip"]}/hp/device/InternalPages/Index?id=UsagePage')
        sn = get_sn(driver)
...
def getCounters(impressoras):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for impressora in impressoras:
            futures.append(executor.submit(getAttrsImpressoras, impressora=impressora))
...
def main():
    ...
    for counter in result:
        if counter["sn"] is not None:
            counterObj = Counters.Counter(counter["printer_id"], counter["total_prints"], counter["total_copies"], counter["total_scans"])
            try:
                counterObj.save(conn)
            except:
                pass
        else:
            print(f'Falha ao salvar a impressora {counter["printer_id"]}. Aparenta estar offline')
